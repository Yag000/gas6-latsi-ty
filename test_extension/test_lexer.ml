...
let test_Var_tokenization =
  let open QCheck in
  Test.make ~count:5000 ~name:"Valid variables are accepted" arbitrary_var
    (fun v -> [ Var v ] = generate_token_list v)

let test_var s =
  test_token_lists
    ("test " ^ s ^ " valid variable tokenization")
    (s |> generate_token_list) [ Var s ]

let test_vars =
  [ "x"; "X"; "ZzZ"; "a_nice_variable"; "is_69"; "test123" ]
  |> List.map test_var
  ...
  run "Lexer_tests"
    [
      ( "test_tokenization",
        ...
        @ test_vars @ test_rem_tokenization @ test_vavers_tokenization
